...
# Mansier, L. & van Rijn, P.C.J. (In press). Modelling agricultural landscape complementation for natural pest control. 
...
lvpred=function(t,n,parms){
  with(as.list(parms), {
    ...
    #MORTALITY
    #Mortality events habitat 2 and 3 (e.g. spraying insecticide)
    etha2=function(t,S){
    ...
    ml=function(t,WNP,Wth){
      ifelse(Wth<x(t) && x(t)<(Wth+1),-log(WNP),0)
    }
...
#setwd("")
#Save data frame
#saveRDS(data_frame_PopDyn, file="ArableLandscape-PestControl.RData")
